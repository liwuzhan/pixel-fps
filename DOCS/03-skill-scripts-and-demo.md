...
状态：首轮单机训练场已实现。每个技能一份脚本，现用弹射背包、自瞄和虚弱榴弹验证即时运动、持续瞄准和投射物附加状态三种行为。
...
这是单机训练场，尚无联机、胜负、撤离或死亡掉落规则。场上提供手枪、步枪、小刀、通用弹药、蓝瓶和三个技能，均需拾取；枪械直接扣除共享弹药，无弹匣与换弹。靶子可移动、还击、复位。当前伤害、耗蓝、冷却、范围与升级公式都是验证数值。
...
| `src/game/skill-runtime.js` | 技能发动检查、资源消耗、冷却、每次发动的运行记录与结束 |
...
5. 针对新增行为验证资源消耗、实际结果与结束清理；执行构建后从 `index.html` 试玩。
