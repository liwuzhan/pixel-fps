...
状态：单机训练场及实验木桩、作弊控制台已实现。每个技能一份脚本，现用弹射背包、自瞄和虚弱榴弹验证即时运动、持续瞄准和投射物附加状态三种行为。新增技能前可以直接布置实验、发放材料并查看结算。
...
这是单机训练场，尚无联机、胜负、撤离或死亡掉落规则。场上提供手枪、步枪、小刀、通用弹药、蓝瓶和三个技能，正常训练中均需拾取，实验台可直接发放；枪械直接扣除共享弹药，无弹匣与换弹。靶子可移动、还击、复位。当前伤害、耗蓝、冷却、范围与升级公式都是验证数值。
...
| `src/game/world.js` | 角色状态、物资与背包、移动碰撞、武器、小刀、投射物命中、伤害、状态、训练场对象 |
...
| `src/game/renderer.js` | 第一人称 WebGL 显示：方块角色、场景、物资、投射物及简单效果 |
| `src/game/app.js` | 输入、界面、暂停、固定步长更新、角色编辑器接入 |
| `build.py` | 将源码内联为两个可离线运行的 HTML 入口 |
| `tests/` | 角色模型、战斗逻辑和技能脚本的自动验证 |

`world.js` 目前汇集多个原型职责，方便直接验证玩法。前面讨论的模块划分仍是职责边界，并不表示所有模块已经独立实现为通用系统。后续按实际复杂度拆分文件。

## 一个技能对应自己的脚本

每份脚本导出一个技能定义，同时注册到 `globalThis.PixelFPSSkills`。构建时先加载 `skill-runtime.js`，再加载各技能脚本；页面创建 `World` 后，将 `new PixelSkillRuntime(world)` 挂到 `world.skills`。
...
2. 为新增编号接入 `world.js` 的初始技能记录与地面掉落物。当前训练场固定列出三种技能，还不是自动扫描脚本生成背包的系统。
3. 在 `src/game/app.js` 及相应界面中添加显示和使用入口；现有快捷键只是这轮 demo 的操作安排，不代表最终技能槽位限制。若技能支持合成，检查合成提示和规则是否适用。
4. 在 `build.py` 和页面脚本占位中加入加载顺序，确保运行入口创建前已经注册该脚本。
5. 针对新增行为验证资源消耗、实际结果与结束清理；执行构建后从 `index.html` 打开实验台，发放该技能并布置木桩。也可用 `lab.give("skill", id, 数量, 等级)` 发放已注册脚本对应的材料，保存场景以便重复检查。
