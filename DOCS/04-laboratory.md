...
| `rocket` | 火箭筒与范围爆炸 | 火箭筒、100 份共享弹药、三个相邻且可复活的木桩，便于观察一发火箭的飞行与范围伤害 |
...
| `give weapon pistol` | 发放一把手枪；武器可选 `pistol`、`rifle`、`knife`、`rocket`，末尾可加数量 |
...
当前技能编号为 `jetpack`、`autoaim`、`weaken`，可用列表动态读取已注册脚本。发放时省略等级为一级，省略数量为一份；普通技能在背包中执行三合一。实验台发放与场地拾取共用背包记账，武器重复发放得到不同实例。
...
状态记录包含添加、移除、到期、来源和到期时间；另外记录成功施法、木桩配置和复活。物资转移记录包含拾取或丢弃、物资类型和数量，武器记录实例编号，技能记录等级。火箭爆炸记录中心、半径与各目标结算结果；实际扣血仍由相同的伤害入口记录。

记录时间为游戏内秒数。状态的剩余时间可以用 `expiresAt - world.time` 得到。场景重新加载后日志清空，需保留结果时先导出日志。

## 保存的是实验配置

配置包含玩家体型与画脸、位置和朝向、武器与技能材料、弹药与蓝瓶、地面物资、每个木桩的体型与行为、全局训练开关、作弊开关、时间倍率与实验暂停状态。

加载时重新生成角色和木桩，恢复满血满蓝，时间从零开始，清除弹丸、状态、技能持续实例、冷却和旧日志。木桩编号重新生成；场景记录当时的木桩坐标，载入后该坐标成为新的移动中心。配置不记录战斗进行到某一瞬间的完整状态。

当前导出格式为 `version: 2`。地面物资逐件保存位置、类型、数量、技能等级等数据；武器在背包和地面之间保留实例编号。已经捡走的物资不会在载入时补回，主动丢弃的物资按保存位置恢复。校验会拒绝背包与地面重复使用同一个武器实例编号的文件，加载后产生的新对象也不会复用已导入编号。

版本 1 文件仍可导入：旧版武器类型列表转为带实例编号的列表，旧版地面物资开关按当时的“整套初始物资 / 空场地”语义迁移。旧文件没有记录逐件拾取状态，无法恢复那部分信息；下一次保存输出版本 2。

通过实验台按钮或文本指令命名保存的场景，在浏览器允许时存入 `localStorage` 的 `fps.lab.scenarios.v1`。这里保存自定义场景集合；加载场景本身不会改写角色编辑器的独立存档。

存储键中的 `v1` 是浏览器场景集合的名称，与单份场景文件的 `version: 2` 分开。技能快捷键属于玩家操作习惯，保存在 `fps.controls.v1`，不随实验场景覆盖。
...
技能施放继续使用 `lab.world.skills.cast(id)`，普通技能合成继续使用 `lab.world.upgradeSkill(id)`。页面还提供 `PixelFPSApp.actions`，供自动化通过与背包、快捷键相同的入口使用物资：
...
这些操作仍检查真实持有量、等级和放置条件，丢弃并捡回技能不会绕过冷却。直接调用绑定方法只改内存；背包界面会另外保存键位。当前仍是单机验证数值；创意清单中的后续技能和物品继续使用这些实验入口验证。
